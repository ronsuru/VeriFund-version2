import {
  ...
  campaignSlotConfigurations,
  type CampaignSlotConfiguration,
  type InsertCampaignSlotConfiguration,
} from "@shared/schema";
import { idGenerator } from "@shared/idUtils";
import { db } from "./db";
import { eq, desc, sql, and, or, not, gt, lt, inArray, isNull, isNotNull, asc, lte, gte } from "drizzle-orm";
...
export class DatabaseStorage implements IStorage {
  ...
  async createCampaign(campaign: InsertCampaign): Promise<Campaign> {
    ...
    // Use user-provided dates instead of calculating endDate from duration
    // Created date is auto-generated by the database
    // Start Date = User's planned start date
    // End Date = User's planned end date
    
    const [newCampaign] = await db
      .insert(campaigns)
      .values({
        ...campaign,
        // Don't override endDate - use the user-provided one
...
// Delete a volunteer report by ID
  ...
  async getCampaignSlotInfo(userId: string): Promise<any> {
    ...
    if (isFirstMonth) {
      ...
      // After first month: apply dynamic credit score tiers
      try {
        const slotConfig = await this.getCampaignSlotConfiguration(avgCreditScore);
        if (slotConfig) {
          maxAllowed = slotConfig.freeSlots;
          paidSlotsAvailable = slotConfig.paidSlotsAvailable;
    ...
    try {
      const allConfigs = await this.getCampaignSlotConfigurations();
      const currentConfig = allConfigs.find(config => 
        config.minCreditScore <= avgCreditScore && 
        config.maxCreditScore >= avgCreditScore && 
        config.isActive
      );
      
      if (currentConfig) {
        const nextConfig = allConfigs.find(config => 
          config.minCreditScore > avgCreditScore && 
          config.isActive
        );
        ...
        if (nextConfig) {
          nextTierInfo = { 
            nextTier: `${nextConfig.minCreditScore}%`, 
            message: `Reach ${nextConfig.minCreditScore}% credit score for ${nextConfig.freeSlots} free slots/month`, 
            requiredScore: nextConfig.minCreditScore 
          };
        }
      }
    } catch (error) {
  ...
  async getFeeConfiguration(transactionType: string): Promise<FeeConfiguration | null> {
    try {
      const [config] = await db
        ...
        .where(
          and(
            eq(feeConfigurations.transactionType, transactionType),
  ...
  async createFeeConfiguration(data: InsertFeeConfiguration): Promise<FeeConfiguration> {
    try {
      const [config] = await db
  ...
  async updateFeeConfiguration(id: string, data: Partial<InsertFeeConfiguration>): Promise<FeeConfiguration> {
    try {
      const [config] = await db
      ...
      const defaultFees = [
        {
          ...
          feePercent: '0.0100', // 1%
          minimumFee: '1.00',
          description: 'Transaction fee for deposits',
        },
        {
          transactionType: 'claim_contribution',
          feePercent: '0.0350', // 3.5%
          minimumFee: '1.00',
          description: 'Platform fee for claiming contributions',
        },
        {
          transactionType: 'claim_tips',
          feePercent: '0.0350', // 3.5%
          minimumFee: '1.00',
          description: 'Platform fee for claiming tips',
        },
        {
          transactionType: 'withdraw',
          feePercent: '0.0100', // 1%
          minimumFee: '1.00',
          description: 'Transaction fee for withdrawals',
        },
      ];

      for (const fee of defaultFees) {
        // Check if fee already exists
        const existing = await this.getFeeConfiguration(fee.transactionType);
        if (!existing) {
          await this.createFeeConfiguration(fee);
        }
      }
  ...
  // Campaign Slot Configuration Methods
  async getCampaignSlotConfigurations(): Promise<CampaignSlotConfiguration[]> {
    try {
      return await db
        .select()
        .from(campaignSlotConfigurations)
        .orderBy(asc(campaignSlotConfigurations.minCreditScore));
    } catch (error) {
  ...
  async getCampaignSlotConfiguration(creditScore: number): Promise<CampaignSlotConfiguration | null> {
    try {
      const [config] = await db
        .select()
        .from(campaignSlotConfigurations)
        .where(
          and(
            lte(campaignSlotConfigurations.minCreditScore, creditScore),
            gte(campaignSlotConfigurations.maxCreditScore, creditScore),
            eq(campaignSlotConfigurations.isActive, true)
          )
        )
        .orderBy(desc(campaignSlotConfigurations.freeSlots))
        .limit(1);
      
      return config || null;
    } catch (error) {
      console.error('Error fetching campaign slot configuration:', error);
      throw error;
    }
  }

  async createCampaignSlotConfiguration(data: InsertCampaignSlotConfiguration): Promise<CampaignSlotConfiguration> {
    try {
      const [config] = await db
        .insert(campaignSlotConfigurations)
        .values(data)
        .returning();
  ...
  async updateCampaignSlotConfiguration(id: string, data: Partial<InsertCampaignSlotConfiguration>): Promise<CampaignSlotConfiguration> {
    try {
      const [config] = await db
        .update(campaignSlotConfigurations)
        .set({
          ...data,
          updatedAt: new Date(),
        })
        .where(eq(campaignSlotConfigurations.id, id))
        .returning();
  ...
  async deleteCampaignSlotConfiguration(id: string): Promise<void> {
    try {
      await db
        .delete(campaignSlotConfigurations)
        .where(eq(campaignSlotConfigurations.id, id));
    } catch (error) {
  ...
  async initializeDefaultCampaignSlots(): Promise<void> {
    try {
      const defaultSlots = [
        {
          tierName: 'New User',
          minCreditScore: -1, // Special tier for new users
          maxCreditScore: -1,
          freeSlots: 10,
          paidSlotsAvailable: 0,
          paidSlotPrice: '0.00',
          description: 'Free slots for new users (first month)',
        },
        {
          tierName: '0-20%',
          minCreditScore: 0,
          ...
          paidSlotsAvailable: 0,
          paidSlotPrice: '0.00',
          description: 'Basic tier for users with 0-20% credit score',
        },
        {
          tierName: '21-35%',
          minCreditScore: 21,
          ...
          paidSlotsAvailable: 0,
          paidSlotPrice: '0.00',
          description: 'Bronze tier for users with 21-35% credit score',
        },
        {
          tierName: '36-50%',
          minCreditScore: 36,
          ...
          paidSlotsAvailable: 0,
          paidSlotPrice: '0.00',
          description: 'Silver tier for users with 36-50% credit score',
        },
        {
          tierName: '51-65%',
          minCreditScore: 51,
          ...
          paidSlotsAvailable: 0,
          paidSlotPrice: '0.00',
          description: 'Gold tier for users with 51-65% credit score',
        },
        {
          tierName: '66-80%',
          minCreditScore: 66,
          ...
          paidSlotsAvailable: 0,
          paidSlotPrice: '0.00',
          description: 'Platinum tier for users with 66-80% credit score',
        },
        {
          tierName: '81-100%',
          minCreditScore: 81,
          ...
          paidSlotsAvailable: 0,
          paidSlotPrice: '0.00',
          description: 'Diamond tier for users with 81-100% credit score',
        },
      ];

      for (const slot of defaultSlots) {
        // Check if configuration already exists for this tier
        const existing = await db
          .select()
          .from(campaignSlotConfigurations)
          .where(eq(campaignSlotConfigurations.tierName, slot.tierName))
          .limit(1);
        
        if (existing.length === 0) {
          await this.createCampaignSlotConfiguration(slot);
        }
      }

      console.log('Default campaign slot configurations initialized');
    } catch (error) {
