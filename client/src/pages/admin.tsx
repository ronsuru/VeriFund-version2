...
{ id: "main", label: "VeriFund", icon: Crown, href: "/admin?tab=main" },
  ...
  const sidenavItems = [
    ...
    { id: "campaign-slots", label: "Campaign Slots", icon: Target },
...
case "notifications": return <NotificationsSection />;      case "tickets": return <TicketsSection />;
      ...
      case "campaign-slots": return <CampaignSlotsSection />;
...
function FeesSection() {
  ...
  const createFeeMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        title: "Fee Created",
        description: "Fee configuration has been created successfully.",
      });
      refetch();
    ...
    onError: (error: any) => {
      toast({
        title: "Error",
        description: error.message || "Failed to create fee configuration",
        variant: "destructive",
      });
    },
  });
  ...
  const updateFeeMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: any }) => {
      const response = await apiRequest('PUT', `/api/admin/fees/${id}`, data);
      return await response.json();
    },
    onSuccess: () => {
      toast({
  ...
  const deleteFeeMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        title: "Error",
        description: error.message || "Failed to delete fee configuration",
        variant: "destructive",
      });
    },
  });
  ...
  const initializeFeesMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest('POST', '/api/admin/fees/initialize');
      return await response.json();
    },
    onSuccess: () => {
      toast({
        title: "Fees Initialized",
        description: "Default fee configurations have been initialized successfully.",
      });
      refetch();
    },
    onError: (error: any) => {
      toast({
        title: "Error",
        description: error.message || "Failed to initialize default fees",
        variant: "destructive",
      });
    },
  });
  ...
  const getFeeTypeLabel = (type: string) => {
    const labels: { [key: string]: string } = {
      ...
      'claim_contribution': 'Platform Fee',
      'claim_tips': 'Platform Fee',
      'withdraw': 'Transaction Fee',
    };
    return labels[type] || 'Fee';
  };
      ...
      <div className="mb-6 sm:mb-8">
        <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
          <div className="flex-1">
            ...
            <p className="mt-1 sm:mt-2 text-sm sm:text-base text-gray-600">Configure platform fees for different transaction types</p>
          </div>
          <div className="flex flex-col space-y-2 sm:flex-row sm:space-y-0 sm:space-x-3">
            <Button
              onClick={() => initializeFeesMutation.mutate()}
              disabled={initializeFeesMutation.isPending}
              variant="outline"
              className="w-full sm:w-auto"
              size="sm"
            >
              {initializeFeesMutation.isPending ? (
                <Loader2 className="h-4 w-4 mr-2 animate-spin" />
              ) : (
                <RotateCcw className="h-4 w-4 mr-2" />
              )}
              <span className="hidden sm:inline">Initialize Defaults</span>
              <span className="sm:hidden">Init Defaults</span>
            </Button>
            <Button
                ...
                <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      ...
                      <Badge variant={fee.isActive ? "default" : "secondary"} className="text-xs sm:text-sm">
                        {fee.isActive ? "Active" : "Inactive"}
                      </Badge>
                    </div>
                    <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-1 truncate">
                      {getFeeTypeLabel(fee.transactionType)}
                    </h3>
                    <p className="text-xs sm:text-sm text-gray-600 mb-2 line-clamp-2">
                      {fee.description || `Fee for ${getTransactionTypeLabel(fee.transactionType).toLowerCase()}`}
                    </p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 text-xs sm:text-sm text-gray-500">
                      <span className="truncate">Fee: {(parseFloat(fee.feePercent) * 100).toFixed(2)}%</span>
                      <span className="truncate">Min: ₱{parseFloat(fee.minimumFee).toFixed(2)}</span>
                      <span className="truncate">Updated: {new Date(fee.updatedAt).toLocaleDateString()}</span>
                      {fee.updatedBy && <span className="truncate">By: {fee.updatedBy}</span>}
                    </div>
                  </div>
      ...
      <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
        <DialogContent className="w-[95vw] max-w-md mx-4 sm:mx-0">
          <DialogHeader>
            ...
            <DialogDescription>
              Add a new fee configuration for a transaction type
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={(e) => {
            e.preventDefault();
            ...
            const data = {
              ...
              feePercent: (parseFloat(formData.get('feePercent') as string) / 100).toFixed(4),
              minimumFee: parseFloat(formData.get('minimumFee') as string).toFixed(2),
              ...
              <div>
                ...
                <Select name="transactionType" required>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="claim_contribution">Claim Contribution</SelectItem>
      ...
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="w-[95vw] max-w-md mx-4 sm:mx-0">
          <DialogHeader>
            ...
            <DialogDescription>
              Update the fee configuration for {selectedFee?.transactionType}
            </DialogDescription>
          </DialogHeader>
          {selectedFee && (
            <form onSubmit={(e) => {
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    value={getTransactionTypeLabel(selectedFee.transactionType)}
                    disabled
                    className="bg-gray-50"
                  />
                  <p className="text-xs text-gray-500 mt-1">Transaction type cannot be changed after creation</p>
                  <input type="hidden" name="transactionType" value={selectedFee.transactionType} />
                </div>
                <div>
                  <Label htmlFor="feePercent">Fee Percentage</Label>
                  <Input
                    ...
                    min="0"
                    max="100"
                    defaultValue={(parseFloat(selectedFee.feePercent) * 100).toFixed(2)}
                    required
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    step="0.01"
                    min="0"
                    defaultValue={parseFloat(selectedFee.minimumFee).toFixed(2)}
                    required
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <Select name="isActive" defaultValue={selectedFee.isActive.toString()}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
...
// Campaign Slots Management Section
function CampaignSlotsSection() {
  const { data: slots, isLoading, refetch } = useQuery({
    queryKey: ['/api/admin/campaign-slots'],
    refetchOnWindowFocus: false,
  });
  const { toast } = useToast();
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  ...
  const createSlotMutation = useMutation({
    mutationFn: async (data: any) => {
      const response = await apiRequest('POST', '/api/admin/campaign-slots', data);
      return await response.json();
    },
    onSuccess: () => {
      toast({
        title: "Slot Configuration Created",
        description: "Campaign slot configuration has been created successfully.",
      });
      refetch();
      setIsCreateModalOpen(false);
    },
    onError: (error: any) => {
      toast({
        title: "Error",
        description: error.message || "Failed to create campaign slot configuration",
        variant: "destructive",
      });
    },
  });
  ...
  const updateSlotMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: any }) => {
      const response = await apiRequest('PUT', `/api/admin/campaign-slots/${id}`, data);
      return await response.json();
    },
    onSuccess: () => {
      toast({
        title: "Slot Configuration Updated",
        description: "Campaign slot configuration has been updated successfully.",
      });
      refetch();
      setIsEditModalOpen(false);
    ...
    onError: (error: any) => {
      toast({
        title: "Error",
        description: error.message || "Failed to update campaign slot configuration",
        variant: "destructive",
      });
    },
  });
  ...
  const deleteSlotMutation = useMutation({
    mutationFn: async (id: string) => {
      await apiRequest('DELETE', `/api/admin/campaign-slots/${id}`);
    },
    onSuccess: () => {
      toast({
        title: "Slot Configuration Deleted",
        description: "Campaign slot configuration has been deleted successfully.",
      });
      refetch();
    },
    onError: (error: any) => {
      toast({
        title: "Error",
        description: error.message || "Failed to delete campaign slot configuration",
        variant: "destructive",
      });
    },
  });

  const initializeSlotsMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest('POST', '/api/admin/campaign-slots/initialize');
      return await response.json();
    },
    onSuccess: () => {
      toast({
        title: "Slots Initialized",
        description: "Default campaign slot configurations have been initialized successfully.",
      });
      refetch();
    },
    onError: (error: any) => {
      toast({
        title: "Error",
        description: error.message || "Failed to initialize default campaign slots",
        variant: "destructive",
      });
    },
  });
  ...
  return (
    <div className="w-full px-4 sm:px-6 lg:px-8">
      <div className="mb-6 sm:mb-8">
        <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
          <div className="flex-1">
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Campaign Slots Management</h1>
            <p className="mt-1 sm:mt-2 text-sm sm:text-base text-gray-600">Configure campaign slot allocations based on credit score tiers</p>
          </div>
          <div className="flex flex-col space-y-2 sm:flex-row sm:space-y-0 sm:space-x-3">
            <Button
              onClick={() => initializeSlotsMutation.mutate()}
              disabled={initializeSlotsMutation.isPending}
              variant="outline"
              className="w-full sm:w-auto"
              size="sm"
            >
              {initializeSlotsMutation.isPending ? (
                <Loader2 className="h-4 w-4 mr-2 animate-spin" />
              ) : (
                <RotateCcw className="h-4 w-4 mr-2" />
              )}
            ...
            >
              <Plus className="h-4 w-4 mr-2" />
              <span className="hidden sm:inline">Add Slot Configuration</span>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-8">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-gray-600"></div>
          <span className="ml-2 text-gray-600">Loading slot configurations...</span>
        </div>
      ) : (
        <div className="grid gap-4 sm:gap-6">
          {slots?.map((slot: any) => (
                ...
                <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      ...
                      <Badge variant={slot.isActive ? "default" : "secondary"} className="text-xs sm:text-sm">
                        {slot.isActive ? "Active" : "Inactive"}
                      </Badge>
                    </div>
                    <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-1 truncate">
                      Credit Score: {slot.minCreditScore === -1 ? 'New User' : `${slot.minCreditScore}-${slot.maxCreditScore}%`}
                    </h3>
                    <p className="text-xs sm:text-sm text-gray-600 mb-2 line-clamp-2">
                      {slot.description || `Slot configuration for ${slot.tierName}`}
                    </p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 text-xs sm:text-sm text-gray-500">
                      <span className="truncate">Free: {slot.freeSlots}</span>
                      <span className="truncate">Paid: {slot.paidSlotsAvailable}</span>
                      <span className="truncate">Price: ₱{parseFloat(slot.paidSlotPrice).toFixed(2)}</span>
                      <span className="truncate">Updated: {new Date(slot.updatedAt).toLocaleDateString()}</span>
                      {slot.updatedBy && <span className="truncate col-span-1 sm:col-span-2 lg:col-span-1">By: {slot.updatedBy}</span>}
                    </div>
                  </div>
                  <div className="flex flex-col space-y-2 sm:flex-row sm:space-y-0 sm:space-x-2 sm:ml-4">
                    <Button
      ...
      <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
        <DialogContent className="w-[95vw] max-w-md mx-4 sm:mx-0">
          <DialogHeader>
            <DialogTitle>Create Slot Configuration</DialogTitle>
            <DialogDescription>
              Add a new campaign slot configuration for a credit score tier
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={(e) => {
            e.preventDefault();
            ...
            const data = {
              ...
              minCreditScore: parseInt(formData.get('minCreditScore') as string),
              maxCreditScore: parseInt(formData.get('maxCreditScore') as string),
              freeSlots: parseInt(formData.get('freeSlots') as string),
              paidSlotsAvailable: parseInt(formData.get('paidSlotsAvailable') as string),
              paidSlotPrice: parseFloat(formData.get('paidSlotPrice') as string).toFixed(2),
              description: formData.get('description') as string,
              isActive: true,
            };
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Label htmlFor="paidSlotsAvailable">Paid Slots Available</Label>
                  <Input
                    name="paidSlotsAvailable"
                    type="number"
                    min="0"
                    placeholder="10"
                    required
              ...
              <div>
                <Label htmlFor="paidSlotPrice">Paid Slot Price (₱)</Label>
                <Input
                  name="paidSlotPrice"
                  type="number"
                  step="0.01"
      ...
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="w-[95vw] max-w-md mx-4 sm:mx-0">
          <DialogHeader>
            <DialogTitle>Edit Slot Configuration</DialogTitle>
            <DialogDescription>
              Update the slot configuration for {selectedSlot?.tierName}
            </DialogDescription>
          </DialogHeader>
          {selectedSlot && (
            <form onSubmit={(e) => {
              ...
              updateSlotMutation.mutate({ id: selectedSlot.id, data });
            }}>
              <div className="space-y-4">
                <div>
                  <Label htmlFor="tierName">Tier Name</Label>
                  <Input
                    name="tierName"
                    value={selectedSlot.tierName}
                    disabled
                    className="bg-gray-50"
                  />
                  <p className="text-xs text-gray-500 mt-1">Tier name cannot be changed after creation</p>
                  <input type="hidden" name="tierName" value={selectedSlot.tierName} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="minCreditScore">Min Credit Score</Label>
                    <Input
                      name="minCreditScore"
                      type="number"
                      defaultValue={selectedSlot.minCreditScore}
                      required
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      name="maxCreditScore"
                      type="number"
                      defaultValue={selectedSlot.maxCreditScore}
                      required
                    />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      ...
                      type="number"
                      min="0"
                      defaultValue={selectedSlot.freeSlots}
                      required
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      type="number"
                      min="0"
                      defaultValue={selectedSlot.paidSlotsAvailable}
                      required
                    />
                  </div>
                </div>
                <div>
                  ...
                  <Input
                    ...
                    step="0.01"
                    min="0"
                    defaultValue={parseFloat(selectedSlot.paidSlotPrice).toFixed(2)}
                    required
                  />
                </div>
                <div>
                ...
                <div>
                  <Label htmlFor="isActive">Status</Label>
                  <Select name="isActive" defaultValue={selectedSlot.isActive.toString()}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
